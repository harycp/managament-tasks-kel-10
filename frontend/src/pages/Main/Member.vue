...
export default {
  ...
  methods: {
    async fetchMembers() {
      ...
      try {
        const response = await axios.get(
          `https://localhost/workspace-service/api/workspaceMembers/${this.workspaceId}`,
    ...
    async handleAddMember() {
      ...
      try {
        await axios.post(
          `https://localhost/workspace-service/api/workspaces/${this.workspaceId}/members`,
          this.newMember,
          { withCredentials: true }
        );
    ...
    async handleRemoveMember(userId) {
      try {
        await axios.delete(
          `https://localhost/workspace-service/api/workspaces/${this.workspaceId}/members/${userId}`,
          { withCredentials: true }
        );
        await this.fetchMembers(); // Muat ulang daftar member
        // Tambahkan notifikasi sukses jika perlu
